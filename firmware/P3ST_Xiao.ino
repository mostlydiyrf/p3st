// Arduino "sketch" for use with the P3ST Xiao Digital VFO/BFO 
...
#define encoderA D0
#define encoderB D1
#define NUMPIXELS 1   // For testing only. Can be removed.
...
//======== GLOBAL DECLARATIONS ===========
//======================================== 
uint32_t calFactor = 160100;       //////*** CHANGE FOR A DIFFERENT CALIBRATION FACTOR AS FOUND FROM
                                   //////*** THE ETHERKIT Si5351_calibration.ino SKETCH AS FOUND IN THE
                                   //////*** ARDUINO IDE: File/Examples/Etherkit Si5351.
uint32_t lastUsedVFO = 14150000;
...
int testProgNum = 5323;            // Used to verify sketch upload. 
int Power = 11;
int PIN  = 12;
...
void bfoFreq() { 
  ...
  while (!button.isLongClick()) {       // Loop until a long press-and-release of encoder button
   ...
   si5351.set_freq(lastUsedBFO * 100, SI5351_CLK2); // Si5351 is set in 0.01 Hz increments. "bfoValue" is in integer Hz.
   lcd.setCursor(7, 1);
  ...
  // save new BFO freq to "EEPROM," set Si5351, and restore the VFO display before returning.
...
////========================================
//void si5351CorrectionFactor(int step) {
//
...
//  lcdClearLine(1);
//  displayFreqLine(1, currentVFOnum);    
//  
//  button.update();
...
//   //si5351.set_freq(lastUsedBFO * 100, SI5351_CLK2); // Si5351 is set in 0.01 Hz increments. "bfoValue" is in integer Hz.
//   lcd.setCursor(0, 1);
//   displayFreqLine(1,currentVFOnum);  //Parameters: LCD line (0 or 1), frequency value.
//    
//    //button.update();
//  } // end while loop
//
//  Serial.println("Yes, it's making it out of the while loop");
//
//  correctionFactor = ((lastUsedVFO - currentVFOnum + 520) * -1);
//  Serial.print("Correction factor: "); Serial.println(correctionFactor);
//  si5351.set_correction(correctionFactor * 100, SI5351_PLL_INPUT_XO);
//  lastUsedVFO = currentVFOnum;
//  Serial.print("lastUsedVFO is "); Serial.println(lastUsedVFO);
//  saveInt(10, correctionFactor);  // Save correctionFactor to eeprom
//  Serial.print("As read from eeprom, the correction factor is "); Serial.println(readInt(10)); 
//  saveInt(10, correctionFactor);
//
