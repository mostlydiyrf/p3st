...
void si5351CorrectionFactor() {
  ...
  int counter;
  uint32_t currentVFOnum = lastUsedVFO + displayOffset;
  ...
  correctionFactor = (((lastUsedVFO + displayOffset) - currentVFOnum) * -1);
  si5351.set_correction(correctionFactor * 100, SI5351_PLL_INPUT_XO);
  lastUsedVFO = currentVFOnum - displayOffset;
  saveInt(10, correctionFactor + 10000);  // Save correctionFactor to eeprom
...
void setup() {
  ...
  si5351.set_correction((calFactor * 100), SI5351_PLL_INPUT_XO);
